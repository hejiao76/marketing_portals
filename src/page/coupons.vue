<template>
  <div>
    ...
    <v-header :showBack="false" :title="pageTitle"></v-header>
    ...
    methods: {
      ...
      getCouponActivityInfo() {
        api.ap_coupon({'activityCode': this.activityCode}).then(res => {
          ...
          document.title=this.pageTitle = res.result.name || "抵扣券活动";
          if (res.status) {
      ...
      Pickclick: function (data) {
        ...
        this.$router.push({path: '/coupon/' + this.activityCode + '/couponsLogin', query: {activecouponId:this.activecouponId,ownerType:this.ownerType,activityName:encodeURI(this.pageTitle)}})
      },
      mesgFn(text){
        this.mesg=text
        var that=this;
